    ...
    <section className="py-20 lg:py-28 bg-background relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-4xl text-center">
        <span className="inline-flex items-center gap-2 bg-wooffy-sky/15 rounded-full px-4 py-2 border border-wooffy-sky/30 mb-6">
          <Store className="w-4 h-4 text-wooffy-blue" />
          <span className="text-sm font-medium text-wooffy-dark">{t("getListed.badge")}</span>
        </span>

        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold text-foreground mb-6">
          {t("getListed.titlePart")} <span className="text-wooffy-blue">{t("getListed.titleHighlight")}</span>
        </h2>

        <p className="text-muted-foreground text-lg mb-3 max-w-xl mx-auto">
          {t("getListed.subtitle")}
        </p>
        <p className="text-xs text-muted-foreground/70 italic mb-10 max-w-xl mx-auto">
          {t("getListed.joke")}
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {[
            ...
            <div
              key={item.title}
              className="bg-card rounded-2xl p-5 border border-border hover:border-wooffy-sky/60 transition-all duration-300 flex flex-col items-center gap-2"
            >
              <div className="w-11 h-11 bg-wooffy-sky/15 rounded-xl flex items-center justify-center">
                <item.icon className="w-5 h-5 text-wooffy-blue" />
              </div>
