...
const ConversionFunnel = ({ businessViews, offerClicks, redemptions }: ConversionFunnelProps) => {
  ...
  const steps = [
    ...
    },
    {
      ...
      width: businessViews > 0 ? `${Math.max(20, (offerClicks / businessViews) * 100)}%` : "20%",
    },
    {
      icon: Gift,
      label: "Redemptions",
      ...
      width: businessViews > 0 ? `${Math.max(10, (redemptions / businessViews) * 100)}%` : "10%",
    },
  ];
              ...
              {index < steps.length - 1 && (
                <div className="flex items-center gap-3 py-1.5">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {index === 0 ? viewToClick : clickToRedeem}% conversion
                  </span>
                </div>
              )}
            </div>
